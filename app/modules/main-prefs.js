...
const MainPrefs = {
  ...
  , getDefaultPrefs: function(){return DATA_PREFS}
    /**
     * Retourne DE FAÇON SYNCHRONE la valeur de la préférence +pid+
     */
  , get: function(pid) {
      return PREFS[pid] || DATA_PREFS[pid].defValue;
    }
    /**
  ...
  , saveIfModified: function(){
      if(true /* this.modified */){this.save()}
    }
    /**
     * Enregistre les nouvelles valeurs de préférences
     * TODO Faire la distinction entre sauvegarde générale et sauvegarde pour
     * l'analyse courante.
     */
  , save: function(){
      log("-> MainPrefs#save")
  ...
  , saveAsPrefsCurrentAnalysis: function(){
      log("-> MainPrefs#saveAsPrefsCurrentAnalysis")
      fs.writeFile(analysisPrefsPath, JSON.stringify(PREFS), (err) => {
        if (err) throw(err)
        log("Préférence analyss sauvegardées avec succès.")
      })
      log("<- MainPrefs#saveAsPrefsCurrentAnalysis")
    }
}
